...
function openModal(html) { const box=document.getElementById("modal-box"); const backdrop=document.getElementById("modal-backdrop"); if(!box||!backdrop)return; box.innerHTML=html; backdrop.classList.remove("hidden"); }
function closeModal() { const box=document.getElementById("modal-box"); const backdrop=document.getElementById("modal-backdrop"); if(box)box.innerHTML=""; if(backdrop)backdrop.classList.add("hidden"); }
...
function bindPageEvents() {
  ...
  if(fileForm) fileForm.onsubmit=async e=>{e.preventDefault(); if(fileForm.dataset.busy==="1")return; fileForm.dataset.busy="1"; const submit=fileForm.querySelector("[type=submit]"); if(submit)submit.disabled=true; try { const data=Object.fromEntries(new FormData(fileForm).entries()); const name=String(data.name||"").trim(); if(!name){showToast("Nama file wajib diisi","error"); return;} const editing=fileForm.dataset.fileId?await getFileById(fileForm.dataset.fileId):null; if(editing){Object.assign(editing,{name,category:String(data.category||"Umum").trim()||"Umum",icon:data.icon||"📁",defaultPercent:Number(data.defaultPercent)||8});await updateFile(editing);} else {data.name=name; data.category=String(data.category||"Umum").trim()||"Umum"; data.defaultPercent=Number(data.defaultPercent)||await getSetting("defaultPercent",8); await createFile(data);} closeModal(); currentPage="files"; currentFileId=null; await renderApp(); showToast("File berhasil dibuat dan disimpan"); } catch(error){ console.error(error); showToast("Gagal menyimpan file. Coba lagi.","error"); } finally {fileForm.dataset.busy="0"; if(submit)submit.disabled=false;} };
  const rowForm=document.getElementById("row-form");
  if(rowForm) rowForm.onsubmit=async e=>{e.preventDefault(); try {const f=await getFileById(currentFileId); if(!f)throw new Error("File tidak ditemukan"); const editingRow=f.rows.find(r=>r.id===rowForm.dataset.rowId); const data=Object.fromEntries(new FormData(rowForm).entries()); if(!String(data.name||"").trim()){showToast("Nama barang wajib diisi","error");return;} await saveRowFromForm(f,rowForm,editingRow?.id||null); closeModal(); await renderApp(); showToast("Data barang berhasil disimpan");} catch(error){console.error(error);showToast("Gagal menyimpan data barang","error");}};
